    ...
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-6 h-full flex items-center justify-between gap-4">
          ...
          <Link
            href="/"
            style={{ fontFamily: 'Syne, sans-serif', color: NAVY, fontWeight: 800, fontSize: '1.25rem' }}
      ...
      {/* Floating WA — hidden when mobile menu is open to prevent overlap.
          Mobile: icon-only circle (w-14 h-14) positioned above disclaimer banner.
          Desktop: pill with text, normal bottom-6 position. */}
      {!open && (
      <a
        href={WA_URL}
        target="_blank"
        ...
        className={[
          ...
          'hover:scale-105 hover:-translate-y-0.5 transition-transform',
          // Mobile: circular icon-only, raised above cookie banner
          'bottom-24 right-4 w-14 h-14 rounded-full',
          // Desktop: pill with text at normal position
          'sm:bottom-6 sm:right-6 sm:w-auto sm:h-auto sm:rounded-[100px] sm:px-4 sm:py-3 sm:gap-2.5',
        ].join(' ')}
      >
        ...
        <span className="hidden sm:inline text-sm font-semibold">Consultar gratis</span>
      </a>
      )}
    </>
  )
